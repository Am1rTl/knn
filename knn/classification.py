...
class KNNClassifier:
    ...
    def _predict_precomputed(self, indices, distances):
        labels_array = np.array([self._labels[index] for index in indices])

        bincount_array = np.array([])
        for label in labels_array:
            bincount = np.bincount(label)
            bincount_array = np.append(bincount_array, np.argmax(bincount))

        return bincount_array
